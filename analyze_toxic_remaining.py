...
for idx, row in df.iterrows():
    ...
    if toxic_prob > 0.3:  # Токсичный
        ...
        # Извлекаем все слова
        words = re.findall(r'[а-яәөүңғқһёa-z]+', detox.lower())
...
print('🎯 Топ-30 слов в ТОКСИЧНЫХ детоксах (паттерны):')
...
print('\n📋 Примеры токсичных детоксов (первые 5):')
for idx, prob, text in toxic_detox[:5]:
    print(f'  [{idx}] P={prob:.2f}: {text[:80]}')

print("\n✅ Готово! Используй эти ПАТТЕРНЫ для улучшения универсального решения.")
